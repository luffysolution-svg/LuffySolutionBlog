...
function SettingsContent() {
  ...
  useEffect(() => {
    const fetchRealConfig = async () => {
      try {
        const res = await fetch('/api/cms/config/get', { cache: 'no-store' });
        ...
        if (data.success && data.data) {
          ...
          setFormData((prev: any) => ({
            ...
            githubComments: { ...(prev.githubComments || {}), ...(data.data.githubComments || {}) },
  ...
  const fetchMusicDetail = async (id: string) => {
    try {
      const res = await fetch(`/api/cms/music/query/${encodeURIComponent(id)}`, { cache: 'no-store' });
      const data = await res.json();
