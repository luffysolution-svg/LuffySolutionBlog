...
function DraftsContent() {
  ...
  const fetchDrafts = async () => {
    setIsLoading(true);
    try {
      const res = await fetch('/api/cms/drafts/list', { cache: 'no-store' });
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch('/api/cms/drafts/delete', {
        method: 'POST',
      ...
      <PageTransition>
        <main className="w-[90%] max-w-5xl mx-auto mt-28 relative z-10">
          ...
          <div className="mb-8 flex gap-4">
            <Link href="/admin/editor?id=new&type=post" className="px-6 py-3 bg-indigo-500 hover:bg-indigo-600 text-white rounded-2xl font-black text-sm shadow-lg shadow-indigo-500/30 transition-all active:scale-95 flex items-center gap-2">
              <Pencil size={16} /> 新建文章草稿
            </Link>
            <Link href="/admin/editor?id=new&type=chatter" className="px-6 py-3 bg-pink-500 hover:bg-pink-600 text-white rounded-2xl font-black text-sm shadow-lg shadow-pink-500/30 transition-all active:scale-95 flex items-center gap-2">
                ...
                {filteredDrafts.map((draft) => (
                  ...
                  >
                    <div className="group bg-white/60 dark:bg-slate-800/50 backdrop-blur-xl border border-white/40 dark:border-white/10 rounded-3xl p-6 shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 overflow-hidden relative">
                      ...
                      <Link href={`/admin/editor?id=${draft.id}`} className="block">
