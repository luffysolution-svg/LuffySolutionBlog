...
function EditorContent() {
  ...
  useEffect(() => {
    const fetchTags = async () => {
      ...
      try {
        const res = await fetch('/api/cms/drafts/all_tags', { cache: 'no-store' });
  ...
  useEffect(() => {
    if (currentDocId !== 'new') {
      const loadDraft = async () => {
        try {
          const res = await fetch('/api/cms/drafts/get', {
            method: 'POST',
  ...
  const handleSave = async (isPublish: boolean, shouldExitAfterSave: boolean = false) => {
    ...
    setIsSaving(true);
    try {
      const res = await fetch('/api/cms/drafts/save', {
      ...
      if (data.success) {
        if (data.id && currentDocId === 'new') {
          ...
          router.replace(`/admin/editor?id=${encodeURIComponent(data.id)}&type=${encodeURIComponent(docType)}`);
        }
        setLastSaved(new Date().toLocaleTimeString());
        setHasUnsavedChanges(false);
        showToast("💾 草稿已同步到云端", "success");
