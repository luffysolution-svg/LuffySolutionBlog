...
export default function AlchemyLab({ posts = [], chatters = [], moments = [] }: any) {
  ...
  useEffect(() => {
    ...
    const fetchGitalkComments = async () => {
      try {
        const [owner, repo] = siteConfig.githubComments.repo.split('/');
        const targetLabel = `workshop-${currentMonthStr}`;
        const issueRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/issues?state=all&per_page=100`);
        const issues = await issueRes.json();
        const targetIssue = Array.isArray(issues)
          ? issues.find((issue: any) => issue.title === targetLabel)
          : null;
        if (targetIssue) {
          const commentsRes = await fetch(targetIssue.comments_url);
